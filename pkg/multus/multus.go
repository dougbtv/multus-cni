...
func CmdAdd(args *skel.CmdArgs, exec invoke.Exec, kubeClient *k8s.ClientInfo) (cnitypes.Result, error) {
	...
	for idx, delegate := range n.Delegates {
		...
		// validate all possible "type" values.
		if strings.ContainsRune(delegate.Conf.Type, os.PathSeparator) {
			return nil, cmdPluginErr(k8sArgs, delegate.Conf.Name, "error adding container to network %q: invalid plugin type '%s'", delegate.Conf.Name, delegate.Conf.Type)
		}

		if delegate.Conf.IPAM != (cnitypes.IPAM{}) {
			if strings.ContainsRune(delegate.Conf.IPAM.Type, os.PathSeparator) {
				return nil, cmdPluginErr(k8sArgs, delegate.Conf.Name, "error adding container to network %q: invalid IPAM plugin type '%s'", delegate.Conf.Name, delegate.Conf.Type)
			}

		}

		if len(delegate.ConfList.Plugins) > 0 {
			for _, plug := range delegate.ConfList.Plugins {
				if strings.ContainsRune(plug.Type, os.PathSeparator) {
					return nil, cmdPluginErr(k8sArgs, delegate.ConfList.Name, "error adding container to network %q: invalid chained plugin type '%s'", delegate.Conf.Name, plug.Type)
				}
				if plug.IPAM != (cnitypes.IPAM{}) {
					if strings.ContainsRune(delegate.Conf.IPAM.Type, os.PathSeparator) {
						return nil, cmdPluginErr(k8sArgs, delegate.ConfList.Name, "error adding container to network %q: invalid chained plugin IPAM type '%s'", delegate.Conf.Name, plug.IPAM.Type)
					}
				}
			}
		}
